...
export namespace IndexedBDSpace {
  /**
   * IndexedBD constants.
   */
  export enum DBKeys {
    /**
     * Login success callback key.
     */
   ...
   */
  export type IDBStore = <T>(txMode: IDBTransactionMode, callback: (store: IDBObjectStore) => T | PromiseLike<T>) => Promise<T>

  /**
   * IDB valid key types.
   */
  export type IDBValidKeyType = IDBValidKey & typeof DBKeys[keyof typeof DBKeys]
}

export function useIndexedDB() {
  const checks = useChecks()
  const [defaultGetStoreFunc, setDefaultGetStoreFunc] = React.useState<IndexedBDSpace.IDBStore | null>(null)
   ...
   */
  function get<T = any>(key: IndexedBDSpace.IDBValidKeyType, customStore: IndexedBDSpace.IDBStore = defaultGetStore()!): Promise<T | undefined> {
  ...
  function set(
    ...
    return customStore('readwrite', (store) => {
      store.put(checks.isString(value) ? value : JSON.stringify(value), key)
   ...
   */
  function entries<KeyType extends IndexedBDSpace.IDBValidKeyType, ValueType = any>(customStore: IndexedBDSpace.IDBStore = defaultGetStore()!): Promise<[KeyType, ValueType][]> {
    return customStore('readonly', async (store) => {
      // Fast path for modern browsers
  ...
  function defaultGetStore() {
    if (!defaultGetStoreFunc)
      setDefaultGetStoreFunc(createStore('database', 'public'))
